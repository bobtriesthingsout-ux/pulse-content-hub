...
async function ingestYouTube(source: typeof sources.$inferSelect) {
  ...
  await db.update(sources).set({ lastCheckedAt: new Date() }).where(eq(sources.id, source.id));
  return { ingested, message: `Ingested ${ingested} new video(s) from ${source.name}` };
}
...
async function ingestRSS(source: typeof sources.$inferSelect) {
  ...
  if (!feedRes.ok) {
    throw new Error(`Failed to fetch RSS feed (HTTP ${feedRes.status}): ${feedUrl}`);
  }
  ...
  for (const item of itemArray) {
    ...
    const guid = typeof guidRaw === "object"
  ...
  await db.update(sources).set({ lastCheckedAt: new Date() }).where(eq(sources.id, source.id));
  return { ingested, message: `Ingested ${ingested} new item(s) from ${source.name}` };
}
...
router.post("/youtube/:sourceId", async (req, res) => {
  try {
    const [source] = await db.select().from(sources).where(eq(sources.id, req.params.sourceId));
